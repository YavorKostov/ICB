...
!      pff_t(:,:) = 0._wp            !             require an initialization of INTENT(out) arguments
      pff_f(:,:) = -1.4_wp * 1E-4_wp            ! CAUTION: set to zero to avoid error with some compilers that
      pff_t(:,:) = -1.4_wp * 1E-4_wp            !             require an initialization of INTENT(out) arguments
      !
